...
void CGameObject::Render(ID3D12GraphicsCommandList *pd3dCommandList, UINT nInstance)
{
	OnPrepareRender();
	...
	if (m_ppMeshes)
	{
		for (int i = 0; i < m_nMeshes; i++)
		{
			if (m_ppMeshes[i]) m_ppMeshes[i]->Render(pd3dCommandList, nInstance);
		}
	}
}
...
void CGameObject::SetLookAt(XMFLOAT3& xmf3Target, XMFLOAT3& xmf3Up)
{
	...
	m_xmf4x4World._31 = xmf3Look.z; m_xmf4x4World._32 = xmf3Look.y; m_xmf4x4World._33 = xmf3Look.z;
...
CHeightMapTerrain::CHeightMapTerrain(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature, LPCTSTR pHeightMapFileName, LPCTSTR pMapFileName, int nWidth, int nLength, int nBlockWidth, int nBlockLength, XMFLOAT3 xmf3Scale, XMFLOAT4 xmf4Color) : CGameObject(0)
{
	...
	pTerrainTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Image/Base_Texture1.dds", 0);
...
{
	m_fRotationAngle = rand() % 150 / 100.0f; 
}
...
{
	if (m_fRotationAngle <= -1.5f) m_fRotationDelta = 1.5f;
	if (m_fRotationAngle >= +1.5f) m_fRotationDelta = -1.5f;
