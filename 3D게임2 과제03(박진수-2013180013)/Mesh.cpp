...
void CStandardMesh::LoadMeshFromFile(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, FILE *pInFile)
{
	...
	for ( ; ; )
	{
		...
		if (!strcmp(pstrToken, "<Bounds>:"))
		{
			...
			m_xmAABB = BoundingBox(m_xmf3AABBCenter, m_xmf3AABBExtents);
		}
...
CHeightMapGridMesh::CHeightMapGridMesh(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, int xStart, int zStart, int nWidth, int nLength, XMFLOAT3 xmf3Scale, XMFLOAT4 xmf4Color, void *pContext0, void *pContext1) : CStandardMesh(pd3dDevice, pd3dCommandList)
{
...
void CHeightMapGridMesh::Render(ID3D12GraphicsCommandList *pd3dCommandList, int nSubSet)
{
	...
	D3D12_VERTEX_BUFFER_VIEW pVertexBufferViews[6] = { m_d3dPositionBufferView, m_d3dColorBufferView, m_d3dNormalBufferView, m_d3dTextureCoord0BufferView, m_d3dTextureCoord1BufferView, m_d3dTextureNumberBufferView };
...
CBillBoardObjectsMesh::CBillBoardObjectsMesh(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList) : CMesh(pd3dDevice, pd3dCommandList)
{
	m_d3dPrimitiveTopology = D3D_PRIMITIVE_TOPOLOGY_POINTLIST;
}

CBillBoardObjectsMesh::~CBillBoardObjectsMesh()
{
	if (m_pd3dSizeBuffer) m_pd3dSizeBuffer->Release();
	if (m_pd3dTextureNumberBuffer) m_pd3dTextureNumberBuffer->Release();

	if (m_pxmf2Sizes) delete[] m_pxmf2Sizes;
	if (m_pnTextureNumbers) delete[] m_pnTextureNumbers;
}

void CBillBoardObjectsMesh::ReleaseUploadBuffers()
{
	CMesh::ReleaseUploadBuffers();
...
void CBillBoardObjectsMesh::Render(ID3D12GraphicsCommandList *pd3dCommandList)
{
	pd3dCommandList->IASetPrimitiveTopology(m_d3dPrimitiveTopology);

	D3D12_VERTEX_BUFFER_VIEW pVertexBufferViews[3] = { m_d3dPositionBufferView, m_d3dSizeBufferView, m_d3dTextureNumberBufferView };
...
CFlowerObjectsMesh::CFlowerObjectsMesh(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, void *pContext) : CBillBoardObjectsMesh(pd3dDevice, pd3dCommandList)
{
	...
	for (int i = 0; i < 5; i++)
	{
		xmf2Size[i] = XMFLOAT2(8.0f + i * 1.0f, 10.0f + i * 1.0f);
	}
	...
	for (int i = 0, x = 0; x < xObjects; x++)
	{
		...
		for (int z = 0; z < zObjects; z++)
		{
			...
			m_pxmf3Positions[i] = XMFLOAT3(xPosition, fHeight, zPosition);
			m_pxmf2Sizes[i] = xmf2Size[i % 5];
			m_pnTextureNumbers[i++] = rand() % 4;
		}
	}
	...
	m_pd3dTextureNumberBuffer = CreateBufferResource(pd3dDevice, pd3dCommandList, m_pnTextureNumbers, sizeof(UINT) * m_nVertices, D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, &m_pd3dTextureNumberUploadBuffer);

	m_d3dTextureNumberBufferView.BufferLocation = m_pd3dTextureNumberBuffer->GetGPUVirtualAddress();
	m_d3dTextureNumberBufferView.StrideInBytes = sizeof(UINT);
	m_d3dTextureNumberBufferView.SizeInBytes = sizeof(UINT) * m_nVertices;
}

CFlowerObjectsMesh::~CFlowerObjectsMesh()
{

}


CTreeObjectsMesh::CTreeObjectsMesh(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, void *pContext) : CBillBoardObjectsMesh(pd3dDevice, pd3dCommandList)
{
	CHeightMapTerrain *pTerrain = (CHeightMapTerrain *)pContext;
	...
	for (int x = 0; x < xObjects; x++)
	{
		...
		for (int z = 0; z < zObjects; z++)
		{
			...
			if (fPixelColor < 100.f || fPixelColor > 250.f)
				m_nVertices--;
		}
	}
	...
	for (int i = 0, x = 0; x < xObjects; x++)
	{
		...
		for (int z = 0; z < zObjects; z++)
		{
			zPosition += fzPitch;
			float fPixelColor = pTerrain->GetColor(xPosition, zPosition);
			if (fPixelColor < 100.0f || fPixelColor > 250.0f) continue;

			float fHeight = pTerrain->GetHeight(xPosition, zPosition);

			m_pxmf3Positions[i] = XMFLOAT3(xPosition, fHeight, zPosition);
			m_pxmf2Sizes[i] = xmf2Size[i % 5];
			m_pnTextureNumbers[i++] = rand() % 5;
		}
	}
...
CTreeObjectsMesh::~CTreeObjectsMesh()
{
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
CRectMesh::CRectMesh(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, float fWidth, float fHeight) : CStandardMesh(pd3dDevice, pd3dCommandList)
{
	m_nVertices = 6;
	m_d3dPrimitiveTopology = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
...
void CRectMesh::Render(ID3D12GraphicsCommandList *pd3dCommandList, int nSubSet)
{
	pd3dCommandList->IASetPrimitiveTopology(m_d3dPrimitiveTopology);
