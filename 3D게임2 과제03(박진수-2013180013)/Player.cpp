...
{
	if (m_pMissailObjects)
	{
		for (int j = 0; j < NUMOFMISSAIL; j++) if (m_pMissailObjects[j]) m_pMissailObjects[j]->Release();
	}
}
...
void CAirplanePlayer::Animate(float fTimeElapsed, XMFLOAT4X4 *pxmf4x4Parent,CCamera *pCamera)
{
	...
	for (int i = 0; i < NUMOFMISSAIL; i++)
	{
		if (m_pMissailObjects[i])
			m_pMissailObjects[i]->Animate(fTimeElapsed, pxmf4x4Parent);
	}
...
void CAirplanePlayer::Render(ID3D12GraphicsCommandList *pd3dCommandList, CCamera *pCamera)
{
	...
	for (int i = 0; i < NUMOFMISSAIL; i++)
	{
		if (m_pMissailObjects[i])
		{
			m_pMissailObjects[i]->UpdateTransform(NULL);
			m_pMissailObjects[i]->Render(pd3dCommandList, pCamera);
		}
	}
...
void CAirplanePlayer::Shot()
{
	if (m_bShotable)
	{
		for (int i = 0; i < NUMOFMISSAIL; i++)
		{
			if (!m_pMissailObjects[i])
			{
				m_pMissailObjects[i] = new CMissailObject();
				m_pMissailObjects[i]->SetPosition(m_xmf3Position);
				m_pMissailObjects[i]->SetRight(GetRight());
				m_pMissailObjects[i]->SetUp(GetUp());
				m_pMissailObjects[i]->SetLook(GetLook());
				m_pMissailObjects[i]->SetChild(m_pHellfireMissileFrame);
				m_pHellfireMissileFrame->AddRef();

				CEffectObject *pEffectObject = new CEffectObject();
				pEffectObject->SetPosition(m_xmf3Position);
				pEffectObject->SetParent(&m_pMissailObjects[i]->m_xmf4x4World);
				pEffectObject->SetOffset(XMFLOAT3(0.0f, 0.0f, -2.0f));
				m_pEffectShader->AddObject(pEffectObject);

				((CMissailObject*)m_pMissailObjects[i])->m_MyEffect = pEffectObject;
